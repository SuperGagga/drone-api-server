...
app.get('/logs/:droneId', async (req, res) => {
  ...
  // ⭐️ 1. (โค้ดใหม่) ดึง page query จาก URL, ถ้าไม่ระบุ ให้เป็น 1
  const page = req.query.page || 1; 

  console.log(`GET /logs for droneId: ${droneId} (Page: ${page})`);

  try {
    // 2. เรียก Server 2 (PocketBase)
    const response = await axios.get(SERVER2_URL, {
      headers: { 'Authorization': `Bearer ${SERVER2_TOKEN}` },
      params: {
        'filter': `(drone_id=${droneId})`,
        'sort': '-created',
        'perPage': 12, // (เรายังคงจำกัด 12 รายการต่อหน้า)
        'page': page   // ⭐️ (โค้ดใหม่) ส่ง page ที่ User ขอ ไปให้ Server 2
      }
    });

    // 3. กรองข้อมูล (ส่วนที่เหลือเหมือนเดิม)
...
/**
 * @path POST /logs
 * @description (ฉบับฉลาด) สร้าง Log ใหม่ใน Server 2
 */
